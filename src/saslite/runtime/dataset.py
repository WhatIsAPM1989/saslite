...
class Dataset:
    ...
    # ``None`` means that provenance has not been classified yet (for
    # example, a dataset freshly read from an XPT backend).  An empty tuple
    # is a closed dataset created by SASLite.  A non-empty tuple identifies
    # the open source schemas whose unknown columns can flow through this
    # dataset in weak-schema mode.
    weak_schema_sources: tuple[str, ...] | None = None
    ...
    def copy(self, deep: bool = True) -> Dataset:
        return Dataset(
            ...
            weak_schema_sources=self.weak_schema_sources,
        )
