...
class ExpressionEvaluator:
    ...
    def _eval_binop(self, node: BinaryOpNode) -> Any:
        ...
        if is_missing(left) or is_missing(right):
            self._diagnose(
                f"missing-generated:{op}",
                f"Missing value generated by operator {op} because an operand "
                "was missing. Handle missing values explicitly before the operation.",
            )
            return float("nan")
        ...
        if op == "/":
            ...
            if r == 0:
                self._diagnose(
                    "missing-generated:division-by-zero",
                    "Missing value generated by division by zero. Validate the "
        ...
        if op == "**":
            try:
                result = self._to_num(left) ** self._to_num(right)
            except (ArithmeticError, ValueError, OverflowError):
                self._diagnose_math_failure("operator **", [left, right])
                return float("nan")
            ...
            return result
        if op == "||":
            if isinstance(left, (int, float)) and not is_missing(left):
                self._diagnose_numeric_to_character(left, "concatenation operator ||")
            if isinstance(right, (int, float)) and not is_missing(right):
                self._diagnose_numeric_to_character(right, "concatenation operator ||")
    ...
    def _eval_func_call(self, node: FunctionCallNode) -> Any:
        ...
        try:
            ...
            if name in {
                "ABS", "CEIL", "COS", "EXP", "FLOOR", "INT", "LOG",
                "LOG10", "MOD", "ROUND", "SIGN", "SIN", "SQRT", "TAN",
            }:
                self._diagnose_math_failure(f"function {name}", args)
                return float("nan")
    ...
    @staticmethod
    def _has_invalid_math_arguments(name: str, args: list[Any]) -> bool:
        """Recognize numeric domains that SAS reports as invalid arguments."""
        try:
            if name == "SQRT" and args and not is_missing(args[0]):
                return float(args[0]) < 0
            if name in {"LOG", "LOG10"} and args and not is_missing(args[0]):
                return float(args[0]) <= 0
            if name == "MOD" and len(args) >= 2 and not is_missing(args[1]):
                return float(args[1]) == 0
        except (TypeError, ValueError):
            return True
        return False

    def _diagnose_math_failure(self, context: str, args: list[Any]) -> None:
        preview = ", ".join(repr(value) for value in args)
        self._diagnose(
            f"invalid-math:{context}:{preview}",
            f"Invalid argument or mathematical operation in {context} for "
            f"value(s) {preview}. SASLite generated a missing value; validate "
            "the function domain before evaluation.",
        )
    ...
    def _compare(self, left: Any, op: str, right: Any) -> bool:
        ...
        if not l_miss and not r_miss and isinstance(left, str) != isinstance(right, str):
            char_value = left if isinstance(left, str) else right
            self._diagnose_character_to_numeric(char_value, f"comparison operator {op}")
    ...
    def _to_num(self, val: Any) -> float:
        ...
        if isinstance(val, str):
            ...
            if val == "" or val == ".":
                return float("nan")
            self._diagnose_character_to_numeric(original, "numeric expression")
            try:
                return float(val)
            except ValueError:
                self._diagnose(
                    f"invalid-numeric:{repr(original)}",
                    f"Invalid numeric data {repr(original)} encountered during "
                    "automatic character-to-numeric conversion. SASLite generated "
                    "a missing value; use INPUT() with an appropriate informat.",
                )
                return float("nan")
        return float("nan")

    def _diagnose_character_to_numeric(self, value: Any, context: str) -> None:
        self._diagnose(
            f"conversion:char-to-num:{context}:{repr(value)}",
            f"Automatic character-to-numeric conversion in {context} for value "
            f"{repr(value)}. Use INPUT() explicitly.",
        )

    def _diagnose_numeric_to_character(self, value: Any, context: str) -> None:
        self._diagnose(
            f"conversion:num-to-char:{context}:{repr(value)}",
            f"Automatic numeric-to-character conversion in {context} for value "
            f"{repr(value)}. Use PUT() explicitly.",
        )
