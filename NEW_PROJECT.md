...
schema-only датасеты и перенос происхождения через `WORK` уже реализованы.
Загрузка dummy fixture CSV как источника строк остаётся следующим этапом.
...
Сгенерированный `saslite-project.json` не перечисляет библиотеки:
...
Количество библиотек заранее неизвестно и ничем не ограничено. Блок
`libraries` в конфиге не используется и считается ошибкой старого формата.
...
## Dummy-данные: следующий этап

Схема и строки остаются независимыми. Планируется один постоянный fixture CSV
на датасет, например `fixtures/SDTM/AE.csv`. Его строки будут накладываться на
полную metadata-схему, а отсутствующие значения останутся missing.
...
До реализации fixture loader не следует создавать адаптированные SAS-программы
или искусственно расширять XPT только ради прохождения schema validation.

## Что ещё предстоит реализовать

- загрузка одного fixture CSV на датасет;
- порядок источников fixture CSV и XPT/SAS7BDAT;
- команда создания fixture из manifest;
